...
# 【重要】ご自身の情報に書き換えてください
# --------------------------------------------------
CHANNEL_ID = 1371823071979372678  # 投稿したいDiscordチャンネルのID（数字）
TOKEN = "ここにボットトークンを貼り付け"  # DiscordのBot Token（文字列）

# リアクションの定義（曜日ごとの絵文字と選択肢）
EMOJIS = ["🔴", "🔵", "🟡", "⚪", "🟣", "🟠", "🟢"]
OPTIONS = [
    "月曜日",
    "火曜日",
...
async def poll(ctx):
    ...
    asyncio.create_task(send_poll())  # ここを安全な実行方法に修正しました


# ボットの起動
if __name__ == "__main__":
    bot.run(TOKEN = "MTU0MzYwOTk1OTgxODg2MjYwMg.GV5FL1.CaTzSCh_RXSTrP1pwiEJVhRvRR2yiOAM77bFTY)
